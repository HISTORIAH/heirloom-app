...
use crate::{
    constants::IKA_DWALLET_PROGRAM_ID, errors::HeirloomIkaError, ika_cpi::CPI_AUTHORITY_SEED,
};
use quasar_lang::prelude::*;

pub fn coordinator_pda() -> Address {
    let seeds: &[&[u8]] = &[b"dwallet_coordinator"];
    match quasar_lang::pda::based_try_find_program_address(seeds, &IKA_DWALLET_PROGRAM_ID) {
        Ok((addr, _)) => addr,
        Err(_) => IKA_DWALLET_PROGRAM_ID,
    }
}

pub fn cpi_authority_pda() -> (Address, u8) {
    let seeds: &[&[u8]] = &[CPI_AUTHORITY_SEED];
    match quasar_lang::pda::based_try_find_program_address(seeds, &crate::ID) {
        Ok((addr, bump)) => (addr, bump),
        Err(_) => (crate::ID, 0), // IT IS AN ERROR SO THE 0 SHOULDN'T MATtER
    }
}

/// Parse a hex ETH address string (with or without 0x prefix) into 20 bytes.
pub fn parse_eth_address_str(s: &[u8]) -> Result<[u8; 20], ProgramError> {
...
fn hex_nibble(b: u8) -> Result<u8, ProgramError> {
    match b {
        b'0'..=b'9' => Ok(b - b'0'),
        b'a'..=b'f' => Ok(b - b'a' + 10),
        b'A'..=b'F' => Ok(b - b'A' + 10),
...
pub fn load_instruction_at(data: &[u8], index: usize) -> Result<(&[u8], &[u8]), ProgramError> {
    if data.len() < 2 {
...
pub fn load_ix_data(data: &[u8], index: usize) -> Result<&[u8], ProgramError> {
