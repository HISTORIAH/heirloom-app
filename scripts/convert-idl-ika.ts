...
const INPUT = join(
  __dirname,
  "../heirloom-ika-program/target/idl/heirloom_ika_program.json",
);
const OUTPUT = join(dirname(INPUT), "heirloom_ika_program.anchor.json");
...
function convertType(type: unknown, optionInner: string = DEFAULT_OPTION_INNER): unknown {
  ...
  // { "vec": { ... } } → recurse into vec inner type
  if ("vec" in t && typeof t.vec === "object") {
    const inner = convertType(t.vec, optionInner);
    return { vec: inner };
  }

  // { "array": [inner, size] } → recurse
  if ("array" in t && Array.isArray(t.array)) {
    const [inner, size] = t.array as [unknown, unknown];
    return { array: [convertType(inner, optionInner), size] };
  }
