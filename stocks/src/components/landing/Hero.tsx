...
export const Hero: React.FC = () => {
  ...
  return (
    <section className="lp-col lp-hero flex flex-col md:block">
      <div className="lp-hero-art order-2 text-foreground">
      ...
      <div className="relative order-1 flex flex-col justify-center pb-10 pt-14 md:min-h-[inherit] md:w-[54%] md:py-24">
        ...
        <p className="lp-lede mt-6 max-w-[30rem] text-foreground/80">{t("landing.hero.lede")}</p>
        <div className="mt-8 flex flex-wrap items-center gap-3">
          <Link to="/portfolio" className="lp-btn lp-btn-primary">
            {t("landing.hero.primary")}
          </Link>
          <Link to="/browse" className="lp-btn lp-btn-ghost">
            {t("landing.hero.secondary")}
          </Link>
        </div>
        <p className="lp-mono-xs mt-6 text-muted-foreground">{t("landing.hero.note")}</p>
      </div>
    </section>
  );
};
