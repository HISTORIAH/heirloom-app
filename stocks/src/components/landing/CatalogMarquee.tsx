...
const Logo: React.FC<{ src: string }> = ({ src }) => {
  ...
  return (
    <img
      ...
      className="lp-logo-mono h-6 w-6 shrink-0 rounded-full border border-tile-line object-cover"
    />
  );
};
...
const Tape: React.FC<{ items: TapeItem[]; reverse?: boolean }> = ({ items, reverse }) => (
  <div className="lp-marquee">
    <div className="lp-marquee-track" data-reverse={reverse || undefined}>
      ...
      {[0, 1].map((copy) => (
        <ul key={copy} className="flex shrink-0" aria-hidden={copy === 1 || undefined}>
          {items.map((item) => (
            <li
              key={item.symbol}
              className="lp-marquee-item flex items-center gap-2.5 px-5 py-2 text-[1.0625rem] font-medium tracking-[-0.01em] text-foreground/80"
            >
...
export const CatalogMarquee: React.FC = () => {
  ...
  return (
    <section className="relative">
      <div className="lp-col flex items-center md:min-h-[16rem]">
        <div className="md:w-1/2 md:pr-10">
          <h2 className="lp-h2 whitespace-pre-line">{t("landing.catalog.title")}</h2>
          ...
          {catalog.isLoading ? (
            ...
            <p className="lp-mono-xs mt-4 text-muted-foreground">
