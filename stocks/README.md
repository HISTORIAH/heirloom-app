...
| `/browse`    | Every catalogued stock, grouped by company, with issuer powers and a Jupiter link; marks what the wallet holds |
...
- `public/catalog.json` is the issuer catalog — names, logos, underlying
  ...
  endpoint than the public one). The catalog labels mainnet stocks, lists those
  whose issuer isn't registered yet, and is what `/browse` shows; balances,
  coverage, and plans always come from the chain.
